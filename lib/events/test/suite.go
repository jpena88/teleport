...
func (s *EventsSuite) SessionEventsCRUD(c *check.C) {
	...
	history, _, err = s.Log.SearchSessionEvents(s.Clock.Now().Add(-1*time.Hour), s.Clock.Now().Add(2*time.Hour), 100, types.EventOrderAscending, "", "alice")
	c.Assert(err, check.IsNil)
	c.Assert(history, check.HasLen, 1)

	history, _, err = s.Log.SearchSessionEvents(s.Clock.Now().Add(-1*time.Hour), s.Clock.Now().Add(2*time.Hour), 100, types.EventOrderAscending, "", "cecile")
	c.Assert(err, check.IsNil)
