...
func (l *AuditLog) SearchSessionEvents(fromUTC, toUTC time.Time, limit int, order types.EventOrder, startKey, withParticipant string) ([]apievents.AuditEvent, string, error) {
	l.log.Debugf("SearchSessionEvents(%v, %v, %v)", fromUTC, toUTC, limit)
	if l.ExternalLog != nil {
		return l.ExternalLog.SearchSessionEvents(fromUTC, toUTC, limit, order, startKey, withParticipant)
	}
	return l.localLog.SearchSessionEvents(fromUTC, toUTC, limit, order, startKey, withParticipant)
}
...
func (a *closedLogger) SearchSessionEvents(fromUTC time.Time, toUTC time.Time, limit int, order types.EventOrder, startKey, withParticipant string) ([]apievents.AuditEvent, string, error) {
	return nil, "", trace.NotImplemented(loggerClosedMessage)
}
