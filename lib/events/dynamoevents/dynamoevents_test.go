...
func (s *DynamoeventsSuite) TestRFD24Migration(c *check.C) {
	...
	err = utils.RetryStaticFor(time.Minute*5, time.Second*5, func() error {
		eventArr, _, err = s.log.searchEventsRaw(start, end, apidefaults.Namespace, 1000, types.EventOrderAscending, "", events.SearchEventsFilter{EventTypes: []string{"test.event"}})
		return err
	})
	c.Assert(err, check.IsNil)
...
func (s *DynamoeventsSuite) TestSessionParticipantsMigration(c *check.C) {
	...
	testParticipants := []string{"test-user1", "test-user2"}
	sessionEndFields, err := json.Marshal(map[string][]string{"participants": testParticipants})
	c.Assert(err, check.IsNil)
	...
	for t := start.Add(time.Minute); t.Before(end); t = t.Add(step) {
		...
		if t.Day()%3 == 0 {
			...
			event.Fields = string(sessionEndFields)
		}
	...
	err = s.log.addSessionParticipantsToSessionEnd(ctx)
	c.Assert(err, check.IsNil)

	eventArr, _, err := s.log.searchEventsRaw(start, end, apidefaults.Namespace, 1000, types.EventOrderAscending, "", events.SearchEventsFilter{EventTypes: []string{events.SessionEndEvent}})
	c.Assert(err, check.IsNil)
	c.Assert(eventArr, check.HasLen, numSessionEnd)

	for _, event := range eventArr {
		c.Assert(event.SessionParticipants, check.DeepEquals, testParticipants)
	}
}
